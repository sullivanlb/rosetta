...
class AppartientSPDAO {
    ...
    public function modification($liaison) {
        if ($liaison instanceof AppartientSP) {
            ...
            // Prépare la déclaration SQL
            $query = "UPDATE AppartientSP SET unScenario=:unScenario, unPack=:unPack, quantite=:quantite WHERE unScenario = '" . $unScenario . "' AND unPack = '" . $unPack . "' AND quantite = '" . $quantite . "'";
            $stmt = $dbc->prepare($query);
