...
class AppartientSQDAO {
    ...
    public function modification($liaison) {
        if ($liaison instanceof AppartientSQ) {
            ...
            // Prépare la déclaration SQL
            $query = "UPDATE AppartientSQ SET unScenario=:unScenario, uneQuestion=:uneQuestion WHERE unScenario = '" . $unScenario . "' AND uneQuestion = '" . $uneQuestion . "'";
            $stmt = $dbc->prepare($query);
            ...
            throw new Exception('AppartientSQDAO: modification(): n est pas une instance de AppartientSQ.');
        } 
    }
}
