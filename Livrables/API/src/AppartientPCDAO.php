...
class AppartientPCDAO {
    ...
    public function modification($liaison) {
        if ($liaison instanceof AppartientPC) {
            ...
            // Prépare la déclaration SQL
            $query = "UPDATE AppartientPC SET unPack=:unPack, unComposant=:unComposant, quantite=:quantite WHERE unPack = '" . $unPack . "' AND unComposant = '" . $unComposant . "' AND quantite = '" . $quantite . "'";
            $stmt = $dbc->prepare($query);
