...
class AppartientDCDAO {
    ...
    public function modification($liaison) {
        if ($liaison instanceof AppartientDC) {
            ...
            // Prépare la déclaration SQL
            $query = "UPDATE AppartientDC SET unDevis=:unDevis, unComposant=:unComposant, quantite=:quantite WHERE unDevis = '" . $unDevis . "' AND unComposant = '" . $unComposant . "' AND quantite = '" . $quantite . "'";
            $stmt = $dbc->prepare($query);
